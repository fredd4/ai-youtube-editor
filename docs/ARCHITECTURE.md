...
1. **Segment pass** (cached by hash of segment spec): for each video segment cut from the normalized source: `-ss in -to out`, apply transform (fit for vertical, crop-pan), grade chain, scale/pad to canvas, `fps=30`, set source-audio gain / mute ranges. Intermediate: `libx264 -crf 16 -preset fast` (or `h264_videotoolbox` for previews) + `pcm_s16le` audio, 48 kHz stereo.
2. **Join pass**: concat demuxer for cuts; `xfade`/`acrossfade` chained for transitions (only when requested; default hard cuts).
