...
| A cutaway muted the user mid-take (six places: dyes, Inca apartment, a local dish, ticket queue, café); the train-price thought cut after "the express train" | The overlay pass only kept audio when the take resumed immediately; a deliberate skip left the cutaway silent; the planner compressed by dropping sentences mid-thought | No-silent-interruption pass: short skips filled with the narration, long skips as J-cuts after the closing sentence; QC rule 36 blocks renders; planner prompt: story first, compress by dropping whole thoughts |
