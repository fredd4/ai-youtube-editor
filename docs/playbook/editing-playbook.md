...
2. **Deterministic tools do the media work; you do the judgment.** Never propose ffmpeg filter strings for the user to run by hand — call the `ytedit` stage that does it. Your output is JSON edits (the cut, the plan) and natural-language decisions/questions.
...
5. **Never silently overwrite a human decision.** `plan/cut.json` is the edit — the source of truth once it exists (`plan/timeline.json` is only the resolved render artifact, regenerated at will). A fresh `plan` run never clobbers a cut with `meta.edited_by_human: true` — it writes `plan/cut.draft.json` and you diff the two for the user. Same principle for anything the user has edited in the web editor.
...
### 2.0 New / ingest
`ytedit new <slug> --language pl`, then the user drops phone clips into `input/`. `ytedit ingest <slug>` builds the mezzanine plus audio/peaks/thumbs and assigns clip IDs in **recording order** (the default travel chronology).

Ingest no longer re-encodes everything. Each source is measured against the project's `format` (`config/defaults.yaml`, overridable in `project.yaml`: codec, pixel format, size, fps — see ARCHITECTURE.md, "Output format"); a clip that already *is* what we render is remuxed in seconds at the same size, and only the rest go through the CRF 16 normalize path (CFR, rotation baked, HDR→SDR). So the stage is only long for the clips that actually need encoding — on iPhone HLG footage that is still all of them, on drone/GoPro/H.264 material it can be nearly none. The run ends with a one-line summary (`mezzanine: 4 copy, 2 encode (vfr 1, hdr 1)`) and a `mezz` column in the clip table; per clip the same lives in `state.json.clips[*].mezzanine`/`.mezzanine_reason`. Read it: an unexpected `encode` reason is a real signal about the footage (a clip shot at 60 fps, a VFR screen recording, a source that is not what the rest of the shoot is).

720p proxies are **not** built by default — they only serve the web editor and `render --draft`, and a remuxed mezzanine is already small. `ytedit ingest <slug> --proxies` builds them up front, `ytedit serve <slug>` builds the missing ones before the editor starts, and a draft render with no proxy falls back to cutting picture from the mezzanine with a warning (correct, just slower). For a project heading into several correction rounds, build them.

**Inspect** `state.json.clips[*]`: orientation, HDR, VFR, duration, has_audio, `mezzanine_reason`; flag anything odd (VFR >5% jitter, missing audio, near-zero duration) before spending money on the next stage. **Ask the user** only if something looks broken (corrupt file, wrong clip count).
...
`ytedit sentences <slug>` runs automatically at the start of `plan` (also runnable on its own): a deterministic pre-pass splits every clip's transcript into numbered sentences (`<clip>#<n>`) and writes `analysis/sentences.json`(+`.md`), flagging each one `instruction` (spoken editor instruction — never usable), `retake_of` (a rejected take attempt — points at the kept sentence), or `duplicate_of` (a near-duplicate of a later sentence anywhere in the project — last-take rule generalized across clips). Nothing picks raw seconds for dialogue any more: a **speech beat** in `plan/cut.json` names a clip and a contiguous run of sentence ids, and the resolver — the one place in the system that turns speech into seconds — derives the cut from the sentence boundaries plus the air. This is what fixed cuts landing inside sentences, narration playing twice, and un-excised retakes: the same sentence cannot be used by two beats, so no audio can play twice, and a beat *is* whole sentences, so no cut can land mid-sentence. A picture-only insert mid-take is a `shots[]` entry on the beat (`after: "<sentence id>"`), never a manually-timed second cut; a shot never carries audio of its own, so the narration keeps running underneath it.

`ytedit plan <slug>` → `plan/edit_plan.json`(+`.md`) and `plan/cut.json` (resolved straight into `plan/timeline.json`) — the big reasoning pass (Opus): beats mapped to timecodes, cold-open picks, music cue sheet, narration requests, title/thumbnail candidates, risk flags. **This is the heaviest review step, not a pass-through.** Before showing the user anything, check: the beats against §4 (real hook at 0:00, promise by 0:07, interrupt by 0:30, something at 3:00/6:00, abrupt end); the cold-open montage picks 2-4 distinct striking moments (not just the first clips in order); pacing against `config/defaults.yaml.pacing` (4s/7s shot ceilings); every analysis `mute_ranges` flag is carried through or dismissed with a reason; narration requests (§5) are short, placed at real gaps, and read naturally; the **"Script check" section of `edit_plan.md`** is clean — any dropped/duplicate/excluded sentence reference there means the planner tried to reuse, retake, or otherwise misreference a sentence and the post-processor had to drop it. **Ask the user:** present `edit_plan.md` as a short summary plus open questions — cuts you're unsure of, footage gaps needing a pickup, title/thumbnail direction. `ytedit plan <slug> --from-response` rebuilds the cut from the previous run's `plan/planner_response.json` with no new LLM call and no cost — use it to pick up a deterministic post-processing fix (e.g. a settings tweak) without re-paying for the same plan.

### 2.3b Resolve & validate (the gate before anything is rendered)
`plan/cut.json` is the edit; `plan/timeline.json` is a derived render artifact that `ytedit resolve <slug>` rewrites from it (and that `render`/`qc`/`at`/`check-render` re-resolve automatically whenever the cut is newer). Never hand-edit `timeline.json` — it is safe to delete and regenerate, and anything you write into it is lost on the next resolve. Edit the cut instead, through a CLI stage or the web editor.

`ytedit validate <slug>` runs the cut validator without writing anything and is what you run after every change you make by hand. Its **errors** block a resolve, so they block every render: an unknown clip or sentence id, a sentence used by two beats (`sentence_reused`), a spoken editor instruction used as narration, a non-contiguous sentence run, a shot range outside its clip, a missing narration file, a pickup whose shots do not cover it (`voice_picture_short`). Its **warnings** are editorial and need your judgment, not a reflex fix: `air_short` (a clip boundary eats into the pad at a speech→speech cut — no amount of padding gives it back), `speech_in_broll` (transcript words inside an ambient B-roll beat: make it a speech beat or mute it), `narrator_visible` (an off-camera beat has a tail no shot covers), `short_piece`, `shot_trimmed`, `retake_sentence`/`duplicate_sentence` (the catalogue's flag is heuristic; using the sentence is your call).

Air around speech is the resolver's job now and is described once, in §4 — there is no separate padding pass to run and nothing to re-snap after an edit.

Never renumber beats or splice the video track by hand in a script. Beat identity is `uid` (assigned once, never changed); `id` (`b001`…) is a display label that is renumbered on every save, and every music cue, caption, chapter and marker resolves through the uid. Reorder or edit beats in the web editor or through a stage; the resolver rebuilds the whole timeline from them.

### 2.4 Review with the user (web editor)
> **Currently down.** `ytedit serve` does not start until `server/` is ported to the cut model (ARCHITECTURE.md, "Web editor"). Until then the user reviews the draft render and sends timecoded notes, and you apply them with `ytedit at` and an edit to `plan/cut.json` (§2.7b). Everything below describes the editor as it will work.

Point him at `http://localhost:8765` to adjust in/out points, reorder, set vertical-clip transforms, mark mute/duck ranges, edit captions/music cues. Once saved, `plan/cut.json` has `meta.edited_by_human: true` — **from here it is the source of truth**; further plan runs write `plan/cut.draft.json` and diff against it, never overwrite it. Re-run the quality-gate checklist (§10) against his edit and flag anything his changes newly violate (e.g. a 9 s A-roll run with no cutaway).
...
Cite `plan/cut.json.markers[]` against this table when reviewing the plan — every listed marker needs a real beat, and a beat with no marker is a sign the plan skipped a structural step.

The pacing ceilings above are subordinate to story continuity, and in cut v2 that is structural rather than policed: a speech beat *is* a run of whole sentences, so a cut can never land mid-sentence, and a shot never carries audio of its own, so a picture insert can never mute the narrator mid-thought. What is left to you is the editorial half — don't drop a sentence out of a thought to hit a shot-length target; compress by dropping whole thoughts.

**Air around speech (the resolver, stated once).** Every speech beat's **picture** gets the full `pacing.speech_pad_before` (0.30 s) before its first word and `pacing.speech_pad_after` (0.45 s) after its last, clamped only by the clip's own edges. The **audio** is pulled back off any neighbouring word: the audible window stops `pacing.word_guard` (0.05 s) short of it, and the extra picture renders silent (`VideoSegment.audio_window`). That is what makes "there is a pause here" a guarantee instead of a hope — the neighbouring word may be on screen for a fraction of a second, but it is never heard. Where the clip boundary itself eats the pad, the air is genuinely short and the validator says so (`air_short`, below `pacing.air_warn_below`, 0.35 s) — that is a footage fact, not something to fix by nudging seconds.
...
That is the whole schema: `file`, and then `request` **or** `after`, plus the optional `label`, `cuts` and `keep_takes`. There is no picture list — the pickup takes over the B-roll beats that already follow its target (below), and when there are none the answer is to choose a shot in the editor, not to name a fallback clip in a manifest.
...
**Placement:** the manifest names the **beat** the pickup goes after (`after: b073`), or a narration `request` id whose request text resolves to one. `ytedit voice` inserts a `voice` beat there — its length is the WAV's own — and takes its picture from the B-roll beats that follow, moving them into the voice beat as its shots (a shot never carries audio of its own, so nothing the pickup talks over is heard underneath unless the beat is deliberately set to `audio: ambient`). When there is no B-roll to take, the report says `voice_picture_short` and the beat is left with no picture for the user to fill in the editor — that is a validation error, so it cannot be rendered by accident. There are no anchors: the pickup is attached to a beat, so nothing can move it under the wrong picture.

**You still review it** — `voice/incoming/report.md` lists every file's cuts (with reasons), final duration, and placement; anything `unplaced` (an unresolved request) or `voice_picture_short` (no B-roll to cover the pickup) needs a manifest fix or a shot chosen in the editor, not a silent skip. Re-running is safe: an unchanged WAV is skipped (tracked by hash in `voice/incoming/state.json`), and a changed one replaces the same beat rather than duplicating it. A run on a human-edited cut writes `plan/cut.draft.json` unless you pass `--force`.
...
- **Location captions:** name + country (+ optional price/temp), 2.0–2.5 s, entering on a cut not a fade, one per new place — **at every new place**, not only the planner's own structural beats; a viewer who wandered in mid-video should always be able to tell where they are. Generated by `ytedit captions <slug>` (`ytedit/ai/locations.py`) once the cut is settled: it canonicalizes the footage log's location names once (`analysis/places.json`), then walks the cut in order and cards every place change not shown in the last `captions.min_gap_s` (default 90 s). Each card is written **onto a beat** in `plan/cut.json` (`{beat, offset, duration}`) rather than pinned to an absolute second, so it follows its picture however the cut is later re-edited — this is the fix for cards that used to land 10-20 s off or under the wrong clip. Re-run it after any edit that reshuffles beats; it's cheap (one writer call, cached) and safe to re-run.
...
**Pacing:** no shot >4 s before 6:00 / >7 s after (rolling, not spot-check, and subordinate to story continuity — a longer take that finishes a thought is fine) · no 12 s stretch without a visual change · no A-roll run >10 s without a cutaway · passes the 1.5× watch test. Silent interruptions of a take and mid-sentence cutaways are no longer possible by construction (§4), so there is no rule left to check for them.

**The cut validator** (`ytedit validate`, also run by `ytedit qc`) replaces the old QC rules 31–37 — the properties they policed are structural in cut v2, and the codes to look for now are `sentence_reused`, `voice_picture_short`, `air_short`, `speech_in_broll`, `narrator_visible`, `short_piece` and `shot_trimmed`. Zero errors is the gate; each warning is an editorial call you make and, where it stands, explain to the user.
